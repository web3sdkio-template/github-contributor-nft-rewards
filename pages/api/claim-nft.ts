...
const claimNft = async (req: NextApiRequest, res: NextApiResponse) => {
  ...
  // 2. Verify the user using web3sdkio Auth
  const web3sdkioUser = await getUser(req);

  // First, get all the public repos of the web3sdkio organization
  ...
  const repos: GithubRepo[] = await fetch(
    `https://api.github.com/orgs/web3sdkio/repos?per_page=100`
  ).then((res) => res.json());

  if (!session || !web3sdkioUser) {
  ...
  for (const repo of reposToCheck) {
    const contributors: GithubContributor[] = await fetch(
      `https://api.github.com/repos/web3sdkio/${repo}/contributors`,
      {
